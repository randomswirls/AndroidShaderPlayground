...
void main( )
{
    ...
    if(ouv.x<.5 && ouv.y >.5)
		gl_FragColor = nightcars3((ouv-vec2(0,.5))*vec2(2,2));
   	else 
    if(ouv.y>.5)
        gl_FragColor = nightcars2((ouv-vec2(.5,.5))*vec2(2,2));
    else 
    if(ouv.x<.5)
        gl_FragColor = nightcars1((ouv-vec2(0,0))*vec2(2,2));
    else 
        gl_FragColor = nightcars4((ouv-vec2(.5,0))*vec2(2,2));
    
    //fragColor = nightcars2((ouv-vec2(0,.5))*vec2(2,2));  
    
    //test
    //fragColor = nightcars3(ouv);
}